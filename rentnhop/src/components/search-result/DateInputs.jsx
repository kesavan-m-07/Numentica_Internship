...
const DateInputs = () => {
  ...
  const updateDifference = appState((state) => state.updateDiffrence);
  const setPickupDate = appState((state) => state.setPickupDate);
  const setReturnDate = appState((state) => state.setReturnDate);
  const {
    watch,
    control,
    handleSubmit,
  ...
  useEffect(() => {
    ...
    updateDifference({days:days,hours});
  ...
  return (
    <form
      ...
      className="flex flex-row w-full sticky bg-white z-10 top-0 shadow-[0_2px_10px_rgba(0,0,0,0.2)] md:shadow-none max-w-[calc(100%-1rem)] rounded-lg m-2 items-end gap-4 font-lufga px-2 py-3 md:p-5"
    >
      <Controller
        name="pickup"
        control={control}
