...
const findCommonNumberInTwoArrays = function (firstUserInput, secondUserInput) {
  ...
  if (
    !Array.isArray(firstUserInput) || !Array.isArray(secondUserInput)
  ) {
  ...
  if (firstUserInput.length === 0 && secondUserInput.length === 0) {
    console.error("Both inputs cannot be empty..");
    return;
  ...
  for (let i = 0; i < firstUserInput.length; i++) {
    ...
    if(!mapToHoldFrequencyOfNumbers[currentElement]){
        mapToHoldFrequencyOfNumbers[currentElement] = 0;
    }
    mapToHoldFrequencyOfNumbers[currentElement] = mapToHoldFrequencyOfNumbers[currentElement]+1;
  }
