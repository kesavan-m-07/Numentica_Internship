...
import PriceBreakup from "../productdetails/PriceBreakup";
import { useAreaVehicleList } from "../../query/useAreaVehicleList";
import { useUpdateCart } from "../../query/useUpdateCart";
...
const CartItemInfo = ({ vehicle }: { vehicle: Vehicle }) => {
  const [isBreakupOpen, setIsBreakUpOpen] = useState(false);
  const { mutate: updateCart } = useUpdateCart();
  
  const { data: allAvailablevehicle } = useAreaVehicleList(vehicle?.areaSlug, vehicle?.bikeSlug);

  if (!vehicle) return null;

  const availableCount = (allAvailablevehicle?.length ?? 0) - (vehicle.quantity ?? 1);
        ...
        <div className="flex-1">
          ...
          <div className="flex justify-between items-center bg-white p-4 rounded-2xl mt-2">
            ...
            <p className="text-customBlue font-semibold text-xs lg:text-base">
              {vehicle.duration?.day ?? 0} days, {vehicle.duration?.hours ?? 0}{" "}
              hrs
            </p>
