...
const VehicleCard = ({ vehicle, selectedAreaOfBike }) => {

  const navigate = useNavigate();
  const difference = appState((state) => state.difference);
  const sortSelected = appState((state) => state.selectedSort);
  ...
  return (
    <div className="font-lufga pt-6 pl-5 shadow-[0_0_10px_rgba(0,0,0,0.3)] my-3 rounded-2xl overflow-hidden space-y-10 hover:-translate-y-2 transition duration-300 flex flex-col justify-between">
      <div className="grid grid-cols-2 pr-5">
        ...
        <div className="flex flex-col items-end">
          {JSON.parse(areaDetails?.tags || "[]").map((tag, i) => (
            <span
              key={i}
      ...
      <div className="grid grid-cols-2">
        <p className="text-lg font-semibold">
          ₹ {areaDetails.pricePerDay * difference.days}
