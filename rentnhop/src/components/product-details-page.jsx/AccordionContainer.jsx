        ...
        <div key={i} className="border border-gray-400 p-4 mx-2 rounded-2xl bg-white my-3">
          ...
          onClick={()=>setOpenedIndex(prev=> prev === i ? null : i)}
           className="flex items-center my-2 cursor-pointer">
            <p className={`font-semibold ${openedIndex === i ? 'text-customBlue underline' : ''} hover:underline`}>{accordion.header}</p>
            <div className={`ml-auto ${openedIndex === i ? 'rotate-180 text-customBlue' : ''} transition-all duration-200`}>{svgIcons.dropDown()}</div>
          </div>
          <ul className={`${openedIndex === i ? 'max-h-[3000px]' : 'max-h-0 '} overflow-hidden px-4 transition-all duration-300 ease-in-out  text-justify list-outside list-disc`}>
            {
              accordion.content.map((content,j)=>(
                <li key={j} className="my-2 text-sm text-gray-500">{content}</li>
              ))
            }
          </ul>
        </div>
